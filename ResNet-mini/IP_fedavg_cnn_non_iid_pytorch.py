...
class ConvNet(torch.nn.Module):
    ...
    def __init__(self, num_classes = 10):
        ...
        self.track_layers = {'conv1': self.residual_block_1.conv_1, 'conv_1_bn': self.residual_block_1.conv_1_bn, 'conv2': self.residual_block_1.conv_2, 'conv_2_bn': self.residual_block_1.conv_2_bn, 'conv_shortcut_1': self.residual_block_1.conv_shortcut_1, 
                            'conv_shortcut_1_bn': self.residual_block_1.conv_shortcut_1_bn, 
                             'conv3': self.residual_block_2.conv_1, 'conv_3_bn': self.residual_block_2.conv_1_bn, 'conv4': self.residual_block_2.conv_2, 'conv_4_bn': self.residual_block_2.conv_2_bn, 'conv_shortcut_2': self.residual_block_2.conv_shortcut_1, 
                            'conv_shortcut_2_bn': self.residual_block_2.conv_shortcut_1_bn, 'linear_1': self.linear_1 }
...
def apply_perturbation(param, delta_c):
        for layer_name in param:
            perturbation_weight = torch.normal(0, delta_c, size = param[layer_name]['weight'].size())
            perturbation_bias = torch.normal(0, delta_c, size = param[layer_name]['bias'].size())
...
for k in ks:
    for x in xs:
        ...
        for r in range(rounds):
            ...
            if r>1:
                 ...
                 if unlearn_prob >= 0.80:
                    ...
                    for prev_round in range(r):
                        #print(prev_round)
                        prev_cluster_file = open("IPFedAvg_noniid cluster indices at {} rounds with k = {} and x = {}.p".format(prev_round, k, x), "rb")
                        ...
                        for i in range(len(prev_cluster_ids)):
                            if unlearn_client_id in prev_cluster_ids[i]:
                                ...
                                pickle.dump(prev_cluster_ids, open("IPFedAvg_noniid cluster indices at {} rounds with k = {} and x = {}.p".format(prev_round, k, x), "wb"))
                    ...
                    if retrain:
                        ...
                        for rU in range(retrain_round, r):
                            ...
                            unlearn_model_file = open("IPFedAvg_noniid global model at {} rounds with k = {} and x = {}.p".format(retrain_round, k, x), "rb")
                            ...
                            for i in range(len(dist_matrix)):
                                for j in range(i, len(dist_matrix)):
                                    temp = get_dist_avg(clients[i].get_weights(), clients[j].get_weights())
                            ...
                            pickle.dump(fedAvg_model.get_parameters(), open("IPFedAvg_noniid global model at {} rounds with k = {}.p".format(rU, k, x), "wb"))
                            pickle.dump(cluster_clientIDs, open("IPFedAvg_noniid cluster indices at {} rounds with k = {} and x = {}.p".format(rU, k, x), "wb"))
                    end = time.time()
            ...
            fedAvg_model.apply_parameters(fedavg_cluster(IP_models, gl_frac))
            pickle.dump(fedAvg_model.get_parameters(), open("IPFedAvg_noniid global model at {} rounds with k = {} and x = {}.p".format(r, k, x), "wb"))
            pickle.dump(cluster_clientIDs, open("IPFedAvg_noniid cluster indices at {} rounds with k = {} and x = {}.p".format(r, k, x), "wb"))
        ...
        pickle.dump(fedavg_test_acc_round, open("IPFedAvg_noniid test accuracy avg after {} rounds with k = {} and x = {}.p".format(rounds, k, x), "wb"))
        pickle.dump(unlearning_time, open("IPFedAvg_noniid unlearning time after {} rounds with k = {} and x = {}.p".format(rounds, k, x), "wb"))